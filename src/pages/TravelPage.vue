<template>
  <main class="page travel-page">
    ...
    <template v-if="travel.state">
      <section v-if="travel.state.active" class="journey" :class="travel.state.active.destination_id">
        <div class="journey-art" aria-hidden="true">{{ destination(travel.state.active.destination_id)?.icon }}<span>··· 🧳 ···</span></div>
        <div><p class="eyebrow">{{ ready ? '远方来信' : '旅途进行中' }}</p><h2>{{ travel.state.active.pet_name }}{{ ready ? '回来啦！' : '正在' + destination(travel.state.active.destination_id)?.name }}</h2>
          <p>{{ ready ? '行李里装着故事，也许还有一份特别的礼物。' : '还需 ' + remaining + ' · 离开页面也会继续旅行' }}</p>
          <progress :value="progress" max="100" aria-label="旅行进度"></progress>
          <p class="muted">预计归来：{{ formatTime(travel.state.active.returns_at) }} · 喂养和老师加分照常进行</p>
      ...
      <section v-else class="departure-note">
        ...
        <label v-if="pets.pets.length">旅行伙伴<select v-model="selectedPet"><option v-for="pet in pets.pets" :key="pet.id" :value="pet.id">{{ pet.name }} · Lv.{{ pet.level }}</option></select></label>
        <RouterLink v-else to="/pet/create">先领养一只宠物 →</RouterLink>
      </section>
      ...
      <section class="destination-grid">
        <article v-for="place in travel.state.destinations" :key="place.id" class="destination-card" :class="place.id">
          ...
          <div class="destination-copy"><small>{{ place.hours }} 小时的慢旅行</small><h3>{{ place.name }}</h3><p>{{ place.description }}</p><div class="reward-tags"><span v-for="item in travel.state.items.filter(i => i.destination_id === place.id)" :key="item.id">{{ item.icon }} {{ item.name }}</span></div>
            <button class="primary" :disabled="disabled || !travel.state.canDepart || !selectedPet" @click="requestDeparture(place)">{{ travel.state.active ? '已有宠物在旅行' : !travel.state.canDepart ? '暂无旅行券' : '使用 1 张旅行券出发 →' }}</button>
          </div>
        </article>
      </section>
      <details class="travel-rules"><summary>旅行与奖励规则</summary><p>完成老师指定的任务获得旅行券，每次出发消耗 1 张，不扣积分。旅行券可积攒、不失效，没有每日出发次数限制；同时只能有一段未领取的旅行，领取行李后有券就能再次出发。奖励可随时领取，不会过期。</p><p>每次保底一张随机明信片和 1 枚印章。另有 1% 概率带回当地装扮（两款各 0.5%）；重复装扮额外转成 2 枚印章。每件专属装扮可用 8 枚印章兑换。</p></details>
...
const ready = computed(() => !!travel.state?.active && now.value >= Date.parse(travel.state.active.returns_at))
const remaining = computed(() => { const minutes = Math.max(1, Math.ceil((Date.parse(travel.state!.active!.returns_at) - now.value) / 60000)); return `${Math.floor(minutes / 60)} 小时 ${minutes % 60} 分钟` })
const progress = computed(() => { const t = travel.state?.active; return t ? Math.max(0, Math.min(100, (now.value - Date.parse(t.started_at)) / (Date.parse(t.returns_at) - Date.parse(t.started_at)) * 100)) : 0 })
...
async function requestDeparture(place: Destination) {
  ...
  if (!pet || disabled.value || !travel.state?.canDepart || pendingDeparture.value) return
...
async function claim() { if (!travel.state?.active) return; notice.value = ''; try { reward.value = await travel.claim(travel.state.active.id); rewardDialog.value?.showModal() } catch (e) { showError(e) } }
