...
assert.deepEqual((await board()).entries.slice(0,2).map(e => e.rank),[1,2])
await db.exec(migration)
...
console.log('PASS: migration replay, feed atomic rollback, award atomic rollback, duplicate requests, ownership, funds, full-hunger feeding, diary daily reward, weekly boundaries, deterministic tie-breaks, >50 students, anonymous-role RPC.')
await db.close()
