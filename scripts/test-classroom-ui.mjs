...
try {
  ...
  await db.query('UPDATE pets SET level=8 WHERE owner_id=$1',[ids[1]])
  ...
  const petWinner=page.locator('.honor-board .rank-item').filter({hasText:'林子墨'})
  await petWinner.locator('.pet-avatar-img').waitFor()
  assert.equal(await petWinner.locator('.is-empty').count(),0,'owned pet must not use the unadopted egg placeholder')
  assert.match(await petWinner.locator('.pet-avatar-img').getAttribute('src'),/baby/)
  await petWinner.locator('.pet-avatar-img').evaluate(img=>img.decode())
  assert.equal(await page.locator('.honor-board .rank-item').filter({hasText:'郑米粒'}).locator('.is-empty').count(),1,'only petless students use the egg placeholder')
  ...
  await awardRow.getByRole('button',{name:'撤销奖励',exact:true}).click()
  ...
  assert.equal(await awardRow.getByRole('button',{name:'撤销奖励',exact:true}).count(),0)
  ...
  await page.locator('.completion-item').filter({hasText:'林子墨'}).getByRole('button',{name:'撤销奖励',exact:true}).click()
  ...
  await page.getByRole('button',{name:'取消',exact:true}).click()
  await page.goto(root+'#/teacher/students',{waitUntil:'domcontentloaded'})
  await page.locator('.student-card').nth(11).waitFor()
  await page.getByRole('checkbox',{name:'选择学生 陈小雨',exact:true}).check()
  assert.equal(await page.locator('.student-card.is-selected').count(),1)
  await page.getByRole('button',{name:'批量发积分',exact:true}).click()
  await page.getByRole('heading',{name:'给已选 1 名学生发积分',exact:true}).waitFor()
  await page.getByRole('button',{name:'取消',exact:true}).click()
  await page.getByRole('button',{name:'清空',exact:true}).click()
  assert.equal(await page.getByRole('button',{name:'批量发积分',exact:true}).isDisabled(),true)
  await page.locator('.student-card').first().getByRole('button',{name:'＋ 发积分',exact:true}).click()
  await page.locator('.dialog').waitFor()
  await page.getByRole('button',{name:'取消',exact:true}).click()
  await page.getByRole('button',{name:'+ 新增学生',exact:true}).click()
  await page.getByPlaceholder('2-12 个字符').waitFor()
  await page.getByRole('button',{name:'取消',exact:true}).click()
  const studentHeights=await page.locator('.student-card').evaluateAll(items=>items.map(el=>el.getBoundingClientRect().height))
  assert.equal(new Set(studentHeights).size,1,'student cards have equal heights')
  await page.screenshot({path:path.join(out,'students-desktop.png')})
  await page.setViewportSize({width:390,height:844})
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,'students mobile has no horizontal overflow')
  await page.screenshot({path:path.join(out,'students-mobile.png')})
