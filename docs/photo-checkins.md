...
现有应用采用自定义密码登录，不具备 Supabase Auth JWT。新功能不把客户端传来的 actor_id 当作照片访问凭证：正常登录或注册成功时，自动用本次输入的密码向 Edge Function 换取随机令牌；打卡页面不再出现单独的密码表单。令牌保存在 localStorage，可跨标签页复用，服务器仅存摘要，12 小时过期；密码不会额外保存。每次请求检查当前密码是否改变，退出时撤销并清除令牌。单账号 15 分钟最多 10 次会话签发尝试。旧版本仅保存账号 ID 的登录状态需重新登录网站一次，完成后自动返回打卡页；会话过期同样返回正常登录页。打卡服务未部署或不可用时不阻断其他功能登录。
